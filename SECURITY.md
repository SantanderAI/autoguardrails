...
`autoguardrails` is an offline-by-default alignment research scaffold: a Python
CLI that evaluates a guardrail `policy.md` against a fixed evaluation suite using
a deterministic local stub. When configured to talk to a real model, it issues
outbound HTTPS requests to an OpenAI-compatible endpoint you supply. Reports
concerning unsafe handling of the protected-surface manifest, path traversal in
the project-root discovery, command injection, or leakage of the configured
`AUTOGUARDRAILS_*_API_KEY` are in scope.
...
- Provide model endpoints and keys exclusively through the `AUTOGUARDRAILS_*` environment variables
